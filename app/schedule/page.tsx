...
export default function SchedulePage() {
  ...
  const handleAddNote = () => {
    ...
    setNoteForm({ title: '', description: '', location: '', duration: '' });
  };
  ...
  const handleOpenNoteEdit = (note: any) => {
  ...
  const handleSaveNoteEdit = () => {
    if (!noteEditData) return;
    setNotes(prev => prev.map(n => (n.id === noteEditData.id ? { ...n, ...noteForm } : n)));
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Notes rendering (в стиле старых зелёных карточек) */}
      {notes.length > 0 && (
              ...
              <div className="mt-2 inline-flex flex-col items-end gap-2 text-sm text-slate-300">
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    ...
                    className="text-cyan-300 hover:text-cyan-400 underline text-xs"
                  >
                    Изменить
                  </button>
                  <button
                    onClick={() => handleDeleteNote(note.id)}
                    className="text-red-300 hover:text-red-400 underline text-xs"
                  >
                    Удалить
                  </button>
                </div>
...
function NoteModal({ title, form, setForm, onSubmit, onClose }: any) {
  const handleChange = (field: string, value: string) =>
    setForm((prev: any) => ({ ...prev, [field]: value }));
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-3xl border border-white/10 bg-slate-900 p-6 text-white relative">
        ...
        <div className="space-y-4">
          ...
          <button
            onClick={onSubmit}
            className="w-full rounded-2xl bg-gradient-to-r from-green-500 to-emerald-400 px-4 py-2 text-sm font-semibold text-white mt-4"
          >
            Сохранить
          </button>
